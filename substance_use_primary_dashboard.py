...
kpi_card = dbc.Card(
   dbc.CardBody([
       html.H4("Discharges related to co-occuring SUD (primary) and MH Disorder (secondary)", className="card-title text-white"),
       html.H2(id="su-primary-kpi-total-deaths", className="text-white"),
   ]),
...
filters_card = dbc.Card(
   dbc.CardBody([
       ...
       html.Label("Substance Type", htmlFor="su-primary-su-filter", tabIndex=2, className="form-label"),
       dcc.Dropdown(
           id="su-primary-su-filter", options=opts_list(su_opts), multi=True,
           placeholder="Substance Type", className="mb-2",
           persistence=True, persistence_type="session"
       ),
       html.Label("Mental Health Diagnosis", htmlFor="su-primary-mh-filter", tabIndex=2, className="form-label"),
       dcc.Dropdown(
           id="su-primary-mh-filter", options=opts_list(mh_opts), multi=True,
           placeholder="Mental Health Diagnosis", className="mb-2",
           persistence=True, persistence_type="session",
           optionHeight=125
       ),
       html.Label("Age Group", htmlFor="su-primary-age-group-filter", tabIndex=6, className="form-label"),
       dcc.Dropdown(
           id="su-primary-age-group-filter", options=opts_list(age_opts), multi=True,
           placeholder="Age Group", className="mb-0",
           persistence=True, persistence_type="session"
       ),
       html.Label("Sex", htmlFor="su-primary-sex-filter", tabIndex=5, className="form-label"),
       dcc.Dropdown(
           id="su-primary-sex-filter", options=opts_list(sex_opts), multi=True,
           placeholder="Sex", className="mb-2",
           persistence=True, persistence_type="session"
       ),
       html.Label("County", htmlFor="su-primary-county-filter", tabIndex=5, className="form-label"),
       dcc.Dropdown(
           id="su-primary-county-filter", options=opts_list(county_opts), multi=True,
           placeholder="County", className="mb-2",
           persistence=True, persistence_type="session"
       ),
       html.Label("Calendar Year", htmlFor="su-primary-year-filter", tabIndex=3, className="form-label"),
       dcc.Dropdown(
           id="su-primary-year-filter", options=opts_list(year_opts), multi=True,
...
def layout_for(
   ...
   center_col = dbc.Col(
       [
           dbc.Row([
               graph_block("su-primary-bar-diagnosis", "Substance Use Diagnosis", bar_h),
               html.P("Bar chart of substance use diagnosis.", className="sr-only"),
           ]),
           dbc.Row([
               graph_block("su-primary-line-discharges", "Discharges by Substance and Year", line_h),
               html.P("Line chart of substance use diagnosis.", className="sr-only"),
           ]),
       ],
       xs=12, md=6
   )
...
# ----------------------------
@callback(
   Output("su-primary-kpi-total-deaths", "children"),
   Output("su-primary-bar-diagnosis", "figure"),
   Output("su-primary-line-discharges", "figure"),
   Output("su-primary-age-group-table", "children"),
   Output("su-primary-gender-table", "children"),
   Output("su-primary-county-table", "children"),
   Input("su-primary-su-filter", "value"),
   Input("su-primary-mh-filter", "value"),
   Input("su-primary-age-group-filter", "value"),
...
def update_dashboard(su, mh, age, sex, county, year):
   ...
   # ---------- Bar chart: Diagnosis by Substance ----------
   if {"record_id", "diagnosis", "diagnosis_type"}.issubset(dff.columns):
       ...
       sub_bar = px.bar(
           ...
           labels={
               ...
               "diagnosis": "Substance Use Diagnosis"
           },
       )
   ...
   if {"record_id", "diagnosis", "diagnosis_type", "year"}.issubset(dff.columns):
       ...
       if substance:
           by_sy["diagnosis"] = pd.Categorical(by_sy["diagnosis"], categories=substance, ordered=True)
