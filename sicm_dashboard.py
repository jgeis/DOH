# sicm_dashboard.py — Stabilization Bed Facilities Occupancy Rates
...
DATA_PATH = Path(__file__).resolve().parent / "data" / "sicm.csv"
...
def _load_sicm_raw():
    sql = load_sql_query("load_sicm_occupancy")
    try:
        df = execute_query(sql)
        print(f"load_sicm_occupancy returned {len(df):,} rows")
    except Exception:
        df = pd.read_csv(DATA_PATH)
        print(f"load_sicm_occupancy fallback csv returned {len(df):,} rows")
    ...
    if missing:
        raise RuntimeError(f"SICM data missing required columns: {missing}")
...
@lru_cache(maxsize=1)
def _load_sicm_dataframe_cached():
    return _load_sicm_raw()


def load_sicm_dataframe():
    return _load_sicm_dataframe_cached().copy()
...
def build_layout():
    ...
    left_col = dbc.Col(
        [
            ...
            last_updated_block,
            html.Div(
                [html.P(text, className="mb-1") for text in sicm_sidebar_text],
    ...
    center_col = dbc.Col(
        [
            html.Div(
                [
                    html.H5("Facility Occupancy Rates", id="sicm-facility-chart-title", className="plot-card-header mb-2"),
                    dcc.Graph(
...
def _build_aggregate_table(dff, view):
    valid_dff = _valid_sicm_frame(dff)
...
@callback(
    Output("sicm-facility-line-chart", "figure"),
    Output("sicm-aggregate-table", "children"),
    Input("sicm-view-toggle", "value"),
    Input("sicm-year-filter", "value"),
    Input("sicm-month-filter", "value"),
    Input("sicm-county-filter", "value"),
    Input("sicm-date-range", "start_date"),
