/****** Object:  View [dbo].[dose_data]    Script Date: 3/3/2026 3:20:44 PM ******/
SET ANSI_NULLS ON
GO
...
/*1 DECLARE DOSE GROUPERS TO USE BASED ON CDC DOSE GROUPER TABLE*/
...
Cocaine AS (SELECT [cocaine_uu] AS Code FROM CDC_DOSE_Groupers),
Methamphetamine AS (
	SELECT meth_icd10data AS Code FROM CDC_DOSE_Groupers
	UNION
	Select [amphetam_uu] FROM CDC_DOSE_Groupers), 
Fentanyl AS (SELECT [fentanyl_uu] AS Code FROM CDC_DOSE_Groupers), 
Heroin AS (SELECT [heroin_uu] AS Code FROM CDC_DOSE_Groupers), 
Benzodiazepine AS (SELECT benzos AS Code FROM CDC_DOSE_Groupers), 
AllDrugs AS (
    ...
    FROM   CDC_DOSE_Groupers
    UNION
    SELECT [narc_synth_other_uu]
    FROM   CDC_DOSE_Groupers
    UNION
    SELECT [narc_unspec_uu]
    FROM   CDC_DOSE_Groupers
    UNION
    SELECT [narc_other_uu]
    FROM   CDC_DOSE_Groupers
    UNION
    SELECT [heroin_uu]
    FROM   CDC_DOSE_Groupers
    ...
    FROM    (SELECT record_id, dx_1, dx_2, dx_3, dx_4, dx_5, dx_6, dx_7, dx_8, dx_9, dx_10, dx_11, dx_12, dx_13, dx_14, dx_15, dx_16, dx_17, dx_18, dx_19, dx_20, dx_21, dx_22, dx_23, dx_24, dx_25
                  FROM    (SELECT record_id, dx_1, dx_2, dx_3, dx_4, dx_5, dx_6, dx_7, dx_8, dx_9, dx_10, dx_11, dx_12, dx_13, dx_14, dx_15, dx_16, dx_17, dx_18, dx_19, dx_20, dx_21, dx_22, dx_23, dx_24, dx_25
                                ...
                                UNION ALL
                                SELECT record_id, dx_1, dx_2, dx_3, dx_4, dx_5, dx_6, dx_7, dx_8, dx_9, dx_10, dx_11, dx_12, dx_13, dx_14, dx_15, dx_16, dx_17, dx_18, dx_19, dx_20, dx_21, dx_22, dx_23, dx_24, dx_25
                                FROM   dbo.Outpatient_DX_2025) AS UnifiedData) AS SourceTable CROSS APPLY(VALUES (dx_1), (dx_2), (dx_3), (dx_4), (dx_5), (dx_6), (dx_7), (dx_8), (dx_9), (dx_10), (dx_11), (dx_12), (dx_13), (dx_14), (dx_15), (dx_16), (dx_17), (dx_18), (dx_19), (dx_20), (dx_21), (dx_22), (dx_23), 
                 (dx_24), (dx_25)) Unpivoted(diagnosis_code)
WHERE diagnosis_code IS NOT NULL AND diagnosis_code LIKE 'T%'


/* Filter applied once here for speed*/ )
